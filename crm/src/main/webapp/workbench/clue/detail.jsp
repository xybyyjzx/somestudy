...
<script type="text/javascript">
	...
	$(function(){
		...
		//页面加载完毕后，展现关联的市场活动信息列表
		showActivityList();

		//为搜索市场活动模态窗口中的文本框绑定敲键盘事件，如果敲的是回车键，则根据名称关键字模糊查询市场活动列表
		//注意：处理模块窗口中文本框的默认敲回车事件（强制刷新页面，将所有的数据抹除掉）
		...
		//为关联按钮绑定事件，执行线索和市场活动的关联操作（关联关系表的添加操作）
		$("#bundBtn").click(function () {

			//参数： 一个clueId+N个activityId
			//传递参数应该以传统方式进行操作，因为activtyId这个key有多条
			//clueId=xxx&activityId=xxx&activityId=xxx&activityId=xxx
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			...
			<!--
				...
				我们当前的detail.jsp跳转到convert.jsp
				convert.jsp需要展现5个数据
				我们以前的做法，展现的数据如果来自于数据库，那么在detail.jsp跳转到convert.jsp期间，一定要经过后台.do的操作取得数据
				但是，今天我们没有这样做
				我们是以在页面跳转的过程中，以传递参数的方式为convert.jsp传递的数据
				这样做起来非常方便

				这样以传参数的形式，为指定页面传递数据，有几个前提条件：

				1.目标页（convert.jsp）所需要的所有的数据,当前页（detail.jsp）都具备
				2.传递的参数的key和value的字符数不能过多，传递的参数基本上都是固定的字符数或者是与名字相关的值，所以我们这个条件也符合
				3.传递的参数中，不允许有安全性相关的参数，例如密码，我们的参数也符合这个条件
